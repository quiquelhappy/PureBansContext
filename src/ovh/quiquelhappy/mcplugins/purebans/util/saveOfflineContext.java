...
public class saveOfflineContext {
    public static context saveCurrentOfflineContext(OfflinePlayer player, Player moderator){
        context context = ovh.quiquelhappy.mcplugins.purebans.util.getOfflineContext.getCurrentOfflineContext(player, moderator);
        ...
        try {
            ...
            stmt.executeUpdate("INSERT INTO pure_bans_context (server, uuid, chat, operator_world, operator_x, operator_y, operator_z, timestamp, player_uuid, operator_uuid) VALUES ('"+main.ServerName+"', '"+uuid+"', '"+chat+"', '"+operator_world+"', "+operator_x+", "+operator_y+", "+operator_z+", "+timestamp+", '"+player_uuid+"', '"+operator_uuid+"');");
